...
/             → LayoutComponent               (guard: authGuard)
  ...
  /charts     → chartsRoutes (lazy — só em desenvolvimento)
  ...
  /ui-elements → uiElementsRoutes (lazy — só em desenvolvimento)
...
| `featureGuard` | `CanActivateFn` | Lê `route.data['feature']`; se ausente ou se `AuthService.hasFeature(feature)` for `true`, permite acesso; caso contrário redireciona para `/403`. Rotas sem `data.feature` são sempre permitidas |
| `authInterceptor` | `HttpInterceptorFn` | Injeta `Authorization: Bearer <token>` em todas as requisições HTTP, exceto `/auth/login` e `/auth/clients` (públicas) |
| `errorInterceptor` | `HttpInterceptorFn` | Em `401` fora de `/auth/login`/`/auth/clients`, limpa a sessão local (`AuthService.clearLocalSession()`, sem chamar `/auth/logout`) e redireciona para `/login`. Em qualquer outro erro, exibe um toast com `error.error.message` (fallback genérico se ausente) |

### Permissionamento por feature

O login retorna `features: string[]` (nomes definidos pelo backend, ex. `PORT_VIEW`, `AREA_EDIT`). `AuthService.hasFeature(name)` é a única fonte de verdade — não há bypass para `superUser`. Duas formas de aplicar:
...
Outro exemplo do mesmo padrão: `ClientPickerDialogComponent` em `src/app/features/auth/pages/login/client-picker-dialog/` — modal que lista os `Client`s (tenants) retornados por `POST /auth/clients` quando o usuário tem acesso a mais de um, resolvendo com o `code` escolhido (ou `undefined` se cancelado) para o `LoginComponent` prosseguir com `POST /auth/login`.
...
| `AuthService` | Gerencia a sessão multi-tenant (login em 2 etapas, logout, feature check); delega chamadas HTTP para `AUTH_REPOSITORY` | Signals (`session`, `isAuthenticated`, `token`) |
...
| `AUTH_REPOSITORY` | `AuthRepository` (abstract) | `app.config.ts` (`AuthHttpRepository`, contra o backend real em `environment.apiUrl`) |
